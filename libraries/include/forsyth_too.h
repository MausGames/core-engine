...
namespace Forsyth
{
    ...
    //-----------------------------------------------------------------------------
    void OptimizeFaces(const uint16* indexList, uint indexCount, uint vertexCount, uint16* __restrict newIndexList, uint16 lruCacheSize);
    ...
    void OptimizeFaces(const uint16* indexList, uint indexCount, uint vertexCount, uint16* __restrict newIndexList, uint16 lruCacheSize)
    {
...

        {
            ...
            activeFaceList.resize(curActiveFaceListPos   +1u);  // # seems to overflow always for one position, only in score-calculation in the find-range though
        }
