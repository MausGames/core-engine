...
#if !defined(LOOP_NONZERO)
    #define LOOP_NONZERO(x) (x)
#endif
...
namespace Forsyth
{
    ...
    void OptimizeFaces(const uint16_t* indexList, uint32_t indexCount, uint32_t vertexCount, uint16_t* __restrict newIndexList, uint16_t lruCacheSize)
    {
        ...
        for (uint32_t i = 0; i < LOOP_NONZERO(indexCount); ++i)
        {
...

        {
            ...
            for (uint32_t i = 0; i < LOOP_NONZERO(vertexCount); ++i)
            {
        ...
        for (uint32_t i = 0; i < LOOP_NONZERO(indexCount); i += 3)
        {
            if (bestScore < 0.0f)
            {
                ...
                for (uint32_t j = 0; j < LOOP_NONZERO(indexCount); j += 3)
                {
            ...
            bestScore = -1.0f;
            for (uint32_t c1 = 0; c1 < LOOP_NONZERO(entriesInCache1); ++c1)
            {
