...
/**Get the total PCM length (number of samples at 48 kHz) of the stream, or of
   ...
   \retval #OP_EINVAL The stream is not seekable (so we can't know the length),
                       \a _li wasn't less than the total number of links in
                       the stream, or the stream was only partially open.*/
OPUS_EXPORT opus_int64 OPUS_CALL op_pcm_total(const OggOpusFile *_of,int _li) OP_ARG_NONNULL(1);
...
/**Obtain the PCM offset of the next sample to be read.
   ...
   \retval #OP_EINVAL The stream was only partially open.*/
OPUS_EXPORT opus_int64 OPUS_CALL op_pcm_tell(const OggOpusFile *_of) OP_ARG_NONNULL(1);
...
OPUS_EXPORT int OPUS_CALL op_pcm_seek(OggOpusFile *_of,opus_int64 _pcm_offset) OP_ARG_NONNULL(1);
