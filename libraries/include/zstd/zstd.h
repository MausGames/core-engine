 ...
 */
ZSTDLIB_API unsigned       ZSTDLIB_CALL ZSTD_isError(size_t result);      /*!< tells if a `size_t` function result is an error code */
ZSTDLIB_API ZSTD_ErrorCode ZSTDLIB_CALL ZSTD_getErrorCode(size_t functionResult); /* convert a result into an error code, which can be compared to error enum list */
ZSTDLIB_API const char*    ZSTDLIB_CALL ZSTD_getErrorName(size_t result); /*!< provides readable string from a function result */
ZSTDLIB_API int            ZSTDLIB_CALL ZSTD_minCLevel(void);             /*!< minimum negative compression level allowed, requires v1.4.0+ */
ZSTDLIB_API int            ZSTDLIB_CALL ZSTD_maxCLevel(void);             /*!< maximum compression level available */
ZSTDLIB_API int            ZSTDLIB_CALL ZSTD_defaultCLevel(void);         /*!< default compression level, specified by ZSTD_CLEVEL_DEFAULT, requires v1.5.0+ */
...
ZSTDLIB_API size_t     ZSTDLIB_CALL ZSTD_freeCCtx(ZSTD_CCtx* cctx);  /* compatible with NULL pointer */
