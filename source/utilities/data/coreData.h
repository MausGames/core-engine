...
public:
    ...
    static coreInt32 PrintBase (coreChar* OUTPUT pcOutput, const coreInt32 iOutputSize, SDL_PRINTF_FORMAT_STRING const coreChar* pcFormat, ...)           SDL_PRINTF_VARARG_FUNC (3);
    static coreInt32 PrintBaseV(coreChar* OUTPUT pcOutput, const coreInt32 iOutputSize, SDL_PRINTF_FORMAT_STRING const coreChar* pcFormat, va_list oArgs) SDL_PRINTF_VARARG_FUNCV(3);
    ...
    static inline const coreChar* DateTimeString(const std::tm* pTimeMap = TIMEMAP_CURRENT) {return coreData::DateTimePrint("%Y-%m-%d %H:%M:%S", pTimeMap);}
...
#if defined(_CORE_MACOS_) || defined(_CORE_EMSCRIPTEN_) || defined(_CORE_SWITCH_)
