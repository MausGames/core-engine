...
void __coreInitOpenGLES()
{
    ...
    std::memset(&g_ContextGLES, 0, sizeof(g_ContextGLES));
    ...
    g_ContextGLES.__Version = coreData::StrVersion(r_cast<const coreChar*>(glGetString(GL_VERSION)));
    g_ContextGLES.__bES30   = (g_ContextGLES.__Version >= corePoint2U8(3u, 0u));
    g_ContextGLES.__bES31   = (g_ContextGLES.__Version >= corePoint2U8(3u, 1u));
    g_ContextGLES.__bES32   = (g_ContextGLES.__Version >= corePoint2U8(3u, 2u));
    ...
    {
        __CORE_GLES_FETCH(glRenderbufferStorageMultisampleAdvanced, AMD, false)
    }

    // implement GL_ANDROID_extension_pack_es31a
    __CORE_GLES_CHECK(GL_ANDROID_extension_pack_es31a, false);
    const coreBool bAndroidPack = g_ContextGLES.__GL_ANDROID_extension_pack_es31a;
    ...
    {
        __CORE_GLES_FETCH(glPolygonMode, ANGLE, false)
    }
    else if(g_sExtensions.contains("GL_NV_polygon_mode "))
    {
        g_ContextGLES.__GL_ANGLE_polygon_mode = true;
        __CORE_GLES_FETCH(glPolygonMode, NV, false)
    }
    ...
    {
        __CORE_GLES_FETCH(glBufferStorage, EXT, false)
    }

    // implement GL_EXT_clear_texture
    if(__CORE_GLES_CHECK(GL_EXT_clear_texture, false))
    {
        __CORE_GLES_FETCH(glClearTexImage, EXT, false)
    }
    ...
    if(bES30)
    {
        g_ContextGLES.__GL_EXT_discard_framebuffer = true;
        g_ContextGLES.__glDiscardFramebuffer = r_cast<PFNGLDISCARDFRAMEBUFFERPROC>(eglGetProcAddress("glInvalidateFramebuffer"));   // override function
    }
    else if(__CORE_GLES_CHECK(GL_EXT_discard_framebuffer, false))
    {
        __CORE_GLES_FETCH(glDiscardFramebuffer, EXT, false)
    }
    ...
    {
        __CORE_GLES_FETCH(glDrawBuffers, EXT, bES30)
    }
    else if(g_sExtensions.contains("GL_NV_draw_buffers ") && g_sExtensions.contains("GL_NV_fbo_color_attachments "))
    {
        g_ContextGLES.__GL_EXT_draw_buffers = true;
        __CORE_GLES_FETCH(glDrawBuffers, NV, false)
    }
    else if(g_sExtensions.contains("GL_WEBGL_draw_buffers "))
    {
        g_ContextGLES.__GL_EXT_draw_buffers = true;
        __CORE_GLES_FETCH(glDrawBuffers, WEBGL, false)
    }

    // implement GL_EXT_instanced_arrays
    if(__CORE_GLES_CHECK(GL_EXT_instanced_arrays, bES30))
    {
        __CORE_GLES_FETCH(glDrawArraysInstanced,   EXT, bES30)
        __CORE_GLES_FETCH(glDrawElementsInstanced, EXT, bES30)
        __CORE_GLES_FETCH(glVertexAttribDivisor,   EXT, bES30)
    }
    else if(g_sExtensions.contains("GL_ANGLE_instanced_arrays "))
    {
        g_ContextGLES.__GL_EXT_instanced_arrays = true;
        __CORE_GLES_FETCH(glDrawArraysInstanced,   ANGLE, false)
        __CORE_GLES_FETCH(glDrawElementsInstanced, ANGLE, false)
        __CORE_GLES_FETCH(glVertexAttribDivisor,   ANGLE, false)
    }
    else if(g_sExtensions.contains("GL_NV_draw_instanced ") && g_sExtensions.contains("GL_NV_instanced_arrays "))
    {
        g_ContextGLES.__GL_EXT_instanced_arrays = true;
        __CORE_GLES_FETCH(glDrawArraysInstanced,   NV, false)
        __CORE_GLES_FETCH(glDrawElementsInstanced, NV, false)
        __CORE_GLES_FETCH(glVertexAttribDivisor,   NV, false)
    }

    // implement GL_EXT_robustness
    if(__CORE_GLES_CHECK(GL_EXT_robustness, bES32))
    {
        __CORE_GLES_FETCH(glGetGraphicsResetStatus, EXT, bES32)
        __CORE_GLES_FETCH(glGetnUniformfv,          EXT, bES32)
        __CORE_GLES_FETCH(glGetnUniformiv,          EXT, bES32)
        __CORE_GLES_FETCH(glReadnPixels,            EXT, bES32)
    }
    else if(g_sExtensions.contains("GL_KHR_robustness "))
    {
        g_ContextGLES.__GL_EXT_robustness = true;
        __CORE_GLES_FETCH(glGetGraphicsResetStatus, KHR, false)
        __CORE_GLES_FETCH(glGetnUniformfv,          KHR, false)
        __CORE_GLES_FETCH(glGetnUniformiv,          KHR, false)
        __CORE_GLES_FETCH(glReadnPixels,            KHR, false)
    }
    ...
    if(g_sExtensions.contains("GL_ANGLE_texture_compression_dxt5 ") &&
       g_sExtensions.contains("GL_EXT_texture_compression_dxt1 "))  g_ContextGLES.__GL_EXT_texture_compression_s3tc = true;
    if(g_sExtensions.contains("GL_NV_texture_compression_s3tc "))   g_ContextGLES.__GL_EXT_texture_compression_s3tc = true;
    if(g_sExtensions.contains("GL_WEBGL_compressed_texture_s3tc ")) g_ContextGLES.__GL_EXT_texture_compression_s3tc = true;
    ...
    {
        __CORE_GLES_FETCH(glTexStorage2D, EXT, bES30)
    }
    ...
    {
        __CORE_GLES_FETCH(glApplyFramebufferAttachmentCMAA, INTEL, false)
    }

    // implement GL_KHR_debug
    if(__CORE_GLES_CHECK(GL_KHR_debug, bES32 || bAndroidPack))
    {
        __CORE_GLES_FETCH(glDebugMessageCallback, KHR, bES32)
        __CORE_GLES_FETCH(glDebugMessageControl,  KHR, bES32)
        __CORE_GLES_FETCH(glObjectLabel,          KHR, bES32)
    }

    // implement GL_KHR_parallel_shader_compile
    if(__CORE_GLES_CHECK(GL_KHR_parallel_shader_compile, false))
    {
        __CORE_GLES_FETCH(glMaxShaderCompilerThreads, KHR, false)
        if(!g_ContextGLES.__glMaxShaderCompilerThreads) g_ContextGLES.__glMaxShaderCompilerThreads = [](GLuint){};   // not available in WebGL
    }
    ...
    {
        __CORE_GLES_FETCH(glCopyBufferSubData, NV, bES30)
    }

    // implement GL_NV_framebuffer_blit
    if(__CORE_GLES_CHECK(GL_NV_framebuffer_blit, bES30))
    {
        __CORE_GLES_FETCH(glBlitFramebuffer, NV, bES30)
    }
    else if(g_sExtensions.contains("GL_ANGLE_framebuffer_blit "))
    {
        g_ContextGLES.__GL_NV_framebuffer_blit = true;
        __CORE_GLES_FETCH(glBlitFramebuffer, ANGLE, false)
    }

    // implement GL_NV_framebuffer_multisample
    if(__CORE_GLES_CHECK(GL_NV_framebuffer_multisample, bES30))
    {
        __CORE_GLES_FETCH(glRenderbufferStorageMultisample, NV, bES30)
    }
    else if(g_sExtensions.contains("GL_ANGLE_framebuffer_multisample "))
    {
        g_ContextGLES.__GL_NV_framebuffer_multisample = true;
        __CORE_GLES_FETCH(glRenderbufferStorageMultisample, ANGLE, false)
    }

    // implement GL_NV_packed_float
    __CORE_GLES_CHECK(GL_NV_packed_float, bES32);
    if(g_sExtensions.contains("GL_EXT_color_buffer_float ")) g_ContextGLES.__GL_NV_packed_float = true;
    ...
    {
        __CORE_GLES_FETCH(glCopyImageSubData, OES, bES32)
    }
    else if(g_sExtensions.contains("GL_EXT_copy_image ") || bAndroidPack)
    {
        g_ContextGLES.__GL_OES_copy_image = true;
        __CORE_GLES_FETCH(glCopyImageSubData, EXT, false)
    }

    // implement GL_OES_depth_texture
    __CORE_GLES_CHECK(GL_OES_depth_texture, bES30);
    if(g_sExtensions.contains("GL_ANGLE_depth_texture ")) g_ContextGLES.__GL_OES_depth_texture = true;
    if(g_sExtensions.contains("GL_WEBGL_depth_texture ")) g_ContextGLES.__GL_OES_depth_texture = true;

    // implement GL_OES_geometry_shader
    __CORE_GLES_CHECK(GL_OES_geometry_shader, bES32 || bAndroidPack);
    if(g_sExtensions.contains("GL_EXT_geometry_shader ")) g_ContextGLES.__GL_OES_geometry_shader = true;

    // implement GL_OES_get_program_binary
    if(__CORE_GLES_CHECK(GL_OES_get_program_binary, bES30))
    {
        __CORE_GLES_FETCH(glGetProgramBinary, OES, bES30)
        __CORE_GLES_FETCH(glProgramBinary,    OES, bES30)
    }

    // implement GL_OES_packed_depth_stencil
    __CORE_GLES_CHECK(GL_OES_packed_depth_stencil, bES30);
    if(g_sExtensions.contains("GL_ANGLE_depth_texture ")) g_ContextGLES.__GL_OES_packed_depth_stencil = true;

    // implement GL_OES_sample_shading
    if(__CORE_GLES_CHECK(GL_OES_sample_shading, bES32 || bAndroidPack))
    {
        __CORE_GLES_FETCH(glMinSampleShading, OES, bES32)
    }

    // implement GL_OES_tessellation_shader
    __CORE_GLES_CHECK(GL_OES_tessellation_shader, bES32 || bAndroidPack);
    if(g_sExtensions.contains("GL_EXT_tessellation_shader ")) g_ContextGLES.__GL_OES_tessellation_shader = true;

    // implement GL_OES_texture_3D
    if(__CORE_GLES_CHECK(GL_OES_texture_3D, bES30))
    {
        __CORE_GLES_FETCH(glTexImage3D,    OES, bES30)
        __CORE_GLES_FETCH(glTexSubImage3D, OES, bES30)
    }
    ...
    {
        __CORE_GLES_FETCH(glBindVertexArray,    OES, bES30)
        __CORE_GLES_FETCH(glDeleteVertexArrays, OES, bES30)
        __CORE_GLES_FETCH(glGenVertexArrays,    OES, bES30)
    }
    ...
    __CORE_GLES_FETCH(glGetBufferSubData, CORE, true)
    if(!g_ContextGLES.__glGetBufferSubData) g_ContextGLES.__glGetBufferSubData = [](GLenum, GLintptr, GLsizeiptr, void*){};   // not available in OpenGL ES

    // map various extensions to GL_CORE_texture_float (# only for half-float)
    if(g_ContextGLES.__GL_OES_texture_half_float && g_ContextGLES.__GL_OES_texture_half_float_linear && (g_ContextGLES.__GL_EXT_color_buffer_float || g_ContextGLES.__GL_EXT_color_buffer_half_float))
    {
