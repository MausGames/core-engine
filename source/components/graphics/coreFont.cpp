...
void coreFont::RetrieveGlyphMetrics(const coreChar32 cGlyph, const coreUint16 iHeight, const coreUint8 iOutline, coreInt32* OUTPUT piMinX, coreInt32* OUTPUT piMaxX, coreInt32* OUTPUT piMinY, coreInt32* OUTPUT piMaxY, coreInt32* OUTPUT piAdvance)
{
    ...
    // retrieve dimensions
    WARN_IF(!TTF_GetGlyphMetrics(m_aapFont.at(iHeight).at(iOutline), cGlyph, piMinX, piMaxX, piMinY, piMaxY, piAdvance)) {}
}
...
coreBool coreFont::__InitHeight(const coreUint16 iHeight, const coreUint8 iOutline)
{
    ...
    WARN_IF(!pNewFont)
    {
        Core::Log->Warning("Sub-Font (%s, height %u, outline %u) could not be loaded", m_sName.c_str(), iHeight, iOutline);
        return false;
    }
    ...
    Core::Log->Info("Sub-Font (%s, height %u, outline %u) loaded", m_sName.c_str(), iHeight, iOutline);
    return true;
}
