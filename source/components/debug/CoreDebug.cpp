...
void CoreDebug::MeasureEnd(const coreHashString& sName)
{
    ...
    if(pMeasure == m_pOverall)
    {
        ...
        if(fTime) pcName = PRINT("%s %.2f FPS%s %.2f MB / %.2f MB", pcName, RCP(fTime), SDL_GL_GetSwapIntervalInline() ? "*" : "", coreDouble(coreData::ProcessMemory()) / (1024.0 * 1024.0), coreDouble(Core::Graphics->ProcessGpuMemory()) / (1024.0 * 1024.0));
    }
