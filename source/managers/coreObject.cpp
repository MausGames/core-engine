...
void coreObjectManager::__Reset(const coreResourceReset eInit)
{
    if(eInit)
    {
        ...
        pBuffer->DefineAttribute(CORE_SHADER_ATTRIBUTE_POSITION_NUM, CORE_VERTEX_SPEC_SNORM_2X16, 0u, 0u);
        ...
        pBuffer->DefineAttribute(CORE_SHADER_ATTRIBUTE_POSITION_NUM, CORE_VERTEX_SPEC_FLOAT_2X32, 0u, 0u);
...
void coreObjectManager::__BindObject(coreObject3D* pObject, const coreInt32 iType)
{
    ...
    FOR_EACH(it, m_aapObjectList)
    {
        ASSERT(!coreData::RangeContains(it->begin(), it->end(), pObject))
    }
