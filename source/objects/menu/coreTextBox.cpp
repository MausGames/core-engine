...
void coreTextBox::Move()
{
    ...
    if(m_bDisplay)
    {
        if(m_cReplace)
        {
            ...
            m_pCaption->SetText(m_bInput ? coreData::Print("%s%c", sPassword.c_str(), m_cCursor) : sPassword.c_str());
        }
        else m_pCaption->SetText(m_bInput ? coreData::Print("%s%c", m_sText.c_str(), m_cCursor) : m_sText.c_str());
